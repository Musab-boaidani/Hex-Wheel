...
$(document).ready(function () {
  ...
  root.style.setProperty(
    "--my-transition",
    " background-color 0.5s, color 0.5s, border 0.5s, width 0.1s, height 0.1s"
  );
  ...
  switch (event.key) {
    ...
    case "ArrowLeft":
      colorPicker.color.hsl = {
        h: currentHue,
        s: 100,
        l: Math.min(currentLightness + HUE_STEP, LIGHTNESS_MAX),
      };
      break;
    case "ArrowRight":
      colorPicker.color.hsl = {
        h: currentHue,
        s: 100,
        l: Math.max(currentLightness - HUE_STEP, LIGHTNESS_MIN),
      };
      break;
    case "Enter":
      $("#guessbtn").click();
...
function AddCurentGuess(hex, result) {
  ...
  for (let i = 1; i <= 6; i++) {
    newElement.append(
      $("<div>", { class: `color-box ${result[i]}` }).text(hex[i])
    );
  }
